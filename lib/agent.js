...
app.post('/library/db-metrics', (request, response) => {
  socket.emit('db-metrics', request.body);
  return response.status(200).json();
});

app.post('/library/custom-logs', (request, response) => {
  socket.emit('custom-logs', request.body);
  return response.status(200).json();
});
...
let statsReader = setInterval(async () => {
  ...
  stats.diskMeasure = 'MB';
  stats.diskFree = Math.round(info.free / 1024 / 1024);
  stats.diskTotal = Math.round(info.total / 1024 / 1024);
  stats.diskAvailable = Math.round(info.available / 1024 / 1024);
  ...
  getCPUUsage(cpu =>
    socket.emit('stats', {
      ...stats,
      cpuUsage: (Math.round(cpu * 100 * 100) / 100) || 0.1
    })
  );
}, INTERVAL_MS);
